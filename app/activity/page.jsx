...
import {
  Heart, Clock, MapPin, Search, LayoutGrid, Building2, Palmtree,
  Anchor, ArrowRight, Car, Waves, Ticket, MessageCircle, Check, Flame, Map
} from "lucide-react";
...
function ActivitiesContent() {
  ...
  // FIXED: Reset count to 10 when filters change
  useEffect(() => {
    setVisibleCount(10);
  ...
  // FIXED: Pass visibleCount to the API limit and extract isFetching
  ...
  const totalItems = data?.data?.total || 0; // Assuming your backend returns total count
    ...
    <div className="space-y-10">
      <div>
        ...
        <div className="space-y-1">
          <button 
            onClick={() => { setActiveCategory(null); setActiveLocation(null); router.push('/activity'); }} 
        ...
        {categories.length > 6 && (
          <button
            onClick={() => visibleCategories >= categories.length ? setVisibleCategories(6) : setVisibleCategories(prev => prev + 6)}
      ...
      <div>
        ...
        <div className="space-y-1">
          <button 
            onClick={() =>{setActiveLocation(null); setActiveCategory(null); router.push(`/activity`);}} 
        ...
        {locations.length > 6 && (
          <button
            onClick={() => visibleLocations >= locations.length ? setVisibleLocations(6) : setVisibleLocations(prev => prev + 6)}
            className="w-full mt-3 py-2 text-xs font-bold text-blue-600 hover:underline"
          >
      ...
      <div className="bg-linear-to-br from-blue-600 to-blue-800 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl shadow-blue-900/10">
        ...
        <a href="https://wa.me/971501902213" target="_blank" rel="noopener noreferrer" className="w-full py-4 bg-white text-blue-600 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 hover:bg-blue-50 transition-all shadow-lg shadow-blue-900/20 active:scale-95">
      ...
      <div className="max-w-350 mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="lg:hidden mb-10 -mx-4 px-4 space-y-4">
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            <button onClick={() => {setActiveCategory(null); setActiveLocation(null); router.push(`/activity`);}} className={`px-5 py-2.5 rounded-xl text-xs font-black whitespace-nowrap border-2 transition-all shadow-sm flex items-center gap-2 ${!activeCategory ? 'bg-slate-950 text-white border-slate-950' : 'bg-white text-slate-500 border-white'}`}>
              <LayoutGrid className="w-4 h-4" /> All
            </button>
            {categories.map(cat => {
               const Icon = getCategoryIcon(cat.name);
               return (
                <button key={cat._id} onClick={() => {setActiveCategory(cat._id); setActiveLocation(null); router.push(`/activity?category=${cat._id}`);}} className={`px-5 py-2.5 rounded-xl text-xs font-black whitespace-nowrap border-2 transition-all shadow-sm flex items-center gap-2 ${activeCategory === cat._id ? 'bg-slate-950 text-white border-slate-950' : 'bg-white text-slate-500 border-white'}`}>
            ...
            {isLoading ? (
              ...
              <>
               <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
                {/* FIXED: Removed .slice() because API handles count now */}
                {filteredActivities.map((tour) => {
                    ...
                    <Link key={tour._id} href={`/activity/${tour._id}`}>
                      <div className="group relative bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col h-full">
                        <div className="relative aspect-4/3 overflow-hidden">
                          ...
                          <span className="absolute bottom-4 left-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-[10px] font-bold text-slate-800 shadow">Adventure</span>
                        </div>
                        <div className="p-6 flex flex-col justify-between flex-1">
                          <div>
                            <h3 className="text-lg font-black text-slate-900 leading-tight mb-1 line-clamp-2 group-hover:text-blue-600 transition">{title}</h3>
                            <p className="text-xs text-slate-500 line-clamp-2 mb-3">{subtitle}</p>
                            <div className="flex items-center gap-4 text-xs text-gray-400 font-bold uppercase">
                              <div className="flex items-center gap-1"><MapPin className="w-3 h-3 text-orange-500" />{location}</div>
                              <div className="flex items-center gap-1"><Clock className="w-3 h-3" />4h</div>
                            </div>
                          </div>
                ...
                {/* FIXED: Load More Button Logic - increments by 10 */}
                {activities.length >= visibleCount && (
                   <div className="mt-16 text-center">
                     <button 
                        onClick={() => setVisibleCount(v => v + 10)} 
                        disabled={isFetching}
                        className="px-12 py-4 bg-white border border-slate-200 text-slate-900 font-black rounded-2xl hover:border-blue-600 hover:text-blue-600 transition-all shadow-sm hover:shadow-xl active:scale-95 disabled:opacity-50"
                      >
                        {isFetching ? "Fetching more..." : "Load More Tours"}
                      </button>
                   </div>
                )}
              </>
              ...
              <div className="bg-white rounded-[3rem] p-20 text-center border border-slate-100 shadow-sm">
                ...
                <button onClick={() => {setActiveCategory(null); setActiveLocation(null); setSearchTerm('');}} className="mt-6 text-blue-600 font-bold hover:underline">Clear all filters</button>
              </div>
            )}
          </div>
        </div>
